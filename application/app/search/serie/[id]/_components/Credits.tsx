...
function DivSvg({cast}: CreditCardProps) {
    const gender: number = cast.gender;
    const svgUrl: string = gender === 1 ? "/assets/images/profile_women.svg" : "/assets/images/profile_men.svg";
    return (
...
function CreditCard({cast}: CreditCardProps) {
    const name: string = cast.isName() ? cast.name : "-";
    const role: string = cast.isRoles() ? (cast.roles[0].isCharacter() ? cast.roles[0].character : "-") : "-";
    ...
    return (
        <li className="my-[10px] ml-[10px] mr-[4px] border-[1px] border-solid border-gray-300 rounded-lg pb-[10px] overflow-hidden min-w-[140px] w-[140px] bg-white shadow-md">
            ...
            <p className="m-0 py-0 px-[10px] text-sm overflow-hidden text-ellipsis">{role}</p>
        </li>
    )
}

function CreditsDiv({credits}: CreditsProps) {
    const casting: Array<CastSerie> = credits.getCasting();
